# evaluate_tuned_bm25.py
"""
Evaluasi BM25 Ranker - TUNED VERSION
Optimized parameters for better MAP, MRR, and F1 scores
"""
...
EVAL_OUTPUT = BASE_DIR / "streamlit_ir/data/eval_v2.json"
...
def evaluate_query(ranker, query, relevant_docs):
    ...
    Args:
        ranker: EnhancedBM25Ranker instance
        query: query string
    ...
    """
    # Search mengembalikan dict dengan key 'results' dan 'query_info'
    search_result = ranker.search(query, top_k=100, verbose=False)
...
def main():
    print("="*80)
    print("EVALUASI BM25 - TUNED VERSION (Optimized Parameters)")
    print("="*80)
    ...
    # Get dictionary stats (manual, karena get_dictionary_stats() error)
    stats = {
        'num_blocks': len(ranker.blocks),
        'num_terms': len(ranker.vocabulary),
        'compression_ratio': len(ranker.vocabulary) / len(ranker.blocks) if ranker.blocks else 0,
        'num_docs': ranker.N,
        'avg_doc_len': ranker.avgdl
    }
