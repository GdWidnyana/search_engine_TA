...
TEST_QUERIES_PATH = Path(__file__).resolve().parent / "qrels.json"
EVAL_OUTPUT = BASE_DIR / "streamlit_ir/data/eval_v1.json"
...
def calculate_ndcg(retrieved, relevant, k=None):
    ...
    # Convert relevant to list if it's a set
    ...
    if k:
        ideal_retrieved = relevant_list[:min(k, len(relevant_list))]
    else:
        ideal_retrieved = relevant_list
...
def calculate_reciprocal_rank(retrieved, relevant):
    ...
    MRR (Mean Reciprocal Rank) is the average of RR scores across multiple queries.
    It measures how well the search system ranks the first relevant result.
    
    Examples:
        Query 1: First relevant at position 1 → RR = 1/1 = 1.000
        Query 2: First relevant at position 3 → RR = 1/3 = 0.333
        Query 3: First relevant at position 5 → RR = 1/5 = 0.200
        Query 4: No relevant found          → RR = 0.000
        
        MRR = (1.000 + 0.333 + 0.200 + 0.000) / 4 = 0.383
    
    Interpretation:
        - MRR = 1.0: Perfect ranking (all queries have relevant doc at position 1)
        - MRR = 0.5: Average first relevant at position 2
        - MRR = 0.333: Average first relevant at position 3
        - MRR = 0.0: No relevant documents found for any query
    
    Use cases:
        - Good for evaluating search systems where users care about the FIRST result
        - Common in web search, question answering, and recommendation systems
        - Less sensitive to total recall than MAP
    
    Args:
        retrieved: list of retrieved doc_ids (in ranking order)
        relevant: list/set of relevant doc_ids
    
    Returns:
        float: Reciprocal Rank score (0.0 to 1.0)
    """
    if not retrieved or not relevant:
        return 0.0
...
def evaluate_query(ranker, query, relevant_docs):
    ...
    Args:
        ranker: ImprovedDictionaryBM25Ranker instance
        query: query string
...
def main():
    ...
    output_data = {
        ...
        'config': {
            'ranker': 'ImprovedDictionaryBM25Ranker',
            'dictionary': {
